...
echo "</div><div>진행중인 미팅:";


$sql2= "SELECT * FROM `meeting` WHERE leader_id='".$row['id']."'";
$result2 = mysqli_query($conn, $sql2);


while($row2=mysqli_fetch_assoc($result2)){
	
		echo "<div id=\"datas_liked\" onclick=\"self.location.href='./meeting_detail.html?id=".$row2['id']."'\">".$row2['project_name']." - ".$row2['meeting_name']."</div>";
	
}

//people liked(3)
$sql3="SELECT * FROM `profile_like_profile` WHERE `profile_liked` = ".$row['id'];
$result3 = mysqli_query($conn, $sql3);

echo "</div><div>나를 좋아요 누른사람 :";

while($row3=mysqli_fetch_assoc($result3)){

	//profile of people liked(4)
	$sql4="SELECT * FROM `profile` WHERE `id` =".$row3['profile_liker'];
...
echo "</div>
     ...
     <input type=\"button\" value=\"쪽지보내기(안보내짐)\" >
